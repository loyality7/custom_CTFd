...
Alpine.data("ChallengeBoard", () => ({
  ...
  // As we use categories to define challenges (we need multiple subtasks), this function computes the total points for a 'category' to render into a card
  async getTotalChallengePoints(category) {
    const f = CTFd.config.themeSettings.sum_challenge_points;

    if (f) {
      const sumChallengePoints = new Function("category", "challenges", f);
      return sumChallengePoints(category, this.challenges);
    }
  ...
  // As we use categories to define challenges (we need multiple subtasks), this function computes the total points for a 'category' to render into a card
  getChallengeDifficulty(category) {
    const m = CTFd.config.themeSettings.challenge_difficulty_map;

    if (m) {
      const challengeDifficultyMap = JSON.parse(m);
      return challengeDifficultyMap[category] || null;
    }
